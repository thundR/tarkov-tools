...
export function getActiveQuests() {
  ...
  let activeQuests = questDataList.filter(
    (quest) =>
      !completedQuestIDs.includes(quest.id) &&
      quest.require.quests.every((requirement) =>
        completedQuestIDs.includes(requirement)
      ) &&
...
export function getCompletedQuests() {
  let completedQuestIDs = localStorage.getCompletedQuestIDs();
  let completedQuests = questDataList.filter((quest) =>
    completedQuestIDs.includes(quest.id)
  );
...
export function getLockedQuests() {
  ...
  let lockedQuests = questDataList.filter(
    (quest) =>
      !quest.require.quests.every((requirement) =>
        completedQuestIDs.includes(requirement)
      ) || userLevel < !quest.require.level
